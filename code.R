library(readxl)
library(dplyr)
data <- read_excel("input_indicatore_22luglio - AGGIORNATO.XLSX")
names(data)[1:3] <- c("regione", "popolazione", "vaccinati")
efficacia <- 0.95


risultati <- data %>% 
  mutate(casi_soglia_50 = popolazione/100000*50, 
...
writexl::write_xlsx(list(input = input, 
                         ...
                         all = risultati), 
                    "risultati_20210722_aggiornato.xlsx")
